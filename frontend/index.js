  ...
  document.querySelector('#citySelect').addEventListener('change', async evt => {
    ...
    try {
      ...
      document.querySelector('#todayDescription')
        .textContent = descriptions.find(d => d[0] === data.curent.weather_descripton) [1]
      document.querySelector('#todayStats div:nth-child(1)')
        .textContent = `${data.curent.temperature_min}*/${data.curent.temperature_max}*`
      document.querySelector('#todayStats div:nth-child(2)')
        .textContent = `Precipitation: ${data.curent.precipitation_probability * 100}%`
      document.querySelector('#todayStats div:nth-child(3)')
        .textContent = `Humidity: ${data.curent.humidity}%`
      document.querySelector('#todayStats div:nth-child(4)')
        .textContent = `Wind: ${data.curent.wind_speed}m/s`
      ...
      data.forecast.daily.forEach((day, idx) => {
        ...
        apparent.textContent = descriptions.find(d => d[0] === day.weather_descripton)[1]
